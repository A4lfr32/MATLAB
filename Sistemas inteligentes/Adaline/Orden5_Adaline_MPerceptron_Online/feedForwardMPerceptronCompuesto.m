function [h, ecm, w, c] = feedForwardMPerceptronCompuesto(alfa,entradas,w,c,no,deseados,entrena,waux,error)
...
%             dL = error.*(h(:,i).*(1 - h(:,i)))*c(:,2:end).*(a2(2:end).*(1 - a2(2:end)))'*waux(1);
%                 dLCapaAnterior = error*(h(:,i).*(1 - h(:,i)))*c(:,2:end).*(a2(2:end).*(1 - a2(2:end)))'*waux;
                dLCapaAnterior = error*waux(4);
                dLCapaActual = ((h(:,i).*(1 - h(:,i)))*c(:,2:end).*(a2(2:end).*(1 - a2(2:end)))')'.*entradas(:,i)';
% Actualizo los pesos Cjk
            
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % Calculo las sensitividades de la capa oculta
            
            c = c + alfa*(dLCapaAnterior*(h(:,i).*(1 - h(:,i)))*a2');
            % Actualizo los pesos Wji
            w = w + alfa*(dLCapaAnterior*dLCapaActual);
